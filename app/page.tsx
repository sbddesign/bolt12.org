      ...
      <main className="flex min-h-screen flex-col items-center pt-20">
        <div className="md:h-screen flex items-center justify-center flex-col gap-8 p-4 py-12 lg:px-12">
          ...
          <div className="flex flex-col items-center text-center  text-xl md:text-2xl lg:text-4xl w-full md:w-2/3 lg:w-1/2 justify-center">
            <span className="text-[#685588] font-headings font-regular">
              Ever wondered what it&rsquo;s like to live in the futuristic
              utopia of BOLT 12?
            </span>
        ...
        <div className="flex flex-col justify-between items-center bg-[url('/blueprint.jpg')] bg-fixed bg-cover w-full">
          <Image
            ...
            height={723}
          />
          <div className="flex flex-col md:gap-20 gap-5 mx-4 my-5 md:mt-12 justify-center">
            ...
            <div className="md:gap-20 gap-5 grid grid-cols-1 md:grid-cols-2 justify-items-center ">
              {nodeImagePaths.map(({ path, description }, index) => (
                <div
                  key={index}
                  className="flex flex-col md:flex-row gap-4 p-4 items-center md:w-[500px] w-full h-full md:h-[180px] bg-white rounded-tl-[96px] rounded-tr-[8px] rounded-br-[48px] rounded-bl-[24px] md:rounded-br-[96px] md:rounded-bl-[48px]"
                >
                  <Image
                    src={path}
                    alt={`Node ${index}`}
        ...
        <div className="w-full flex flex-col gap-12 py-16 items-center justify-center">
          <p className="font-headings text-b12-purple text-center mx-auto text-5xl">
            Now let’s see BOLT 12{" "}
            <span className="font-display block text-6xl">in action!</span>
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 p-6 gap-x-12 gap-y-16 max-w-7xl">
            {projects.map((project, index) => (
              <>
                <div
                  className="flex flex-col gap-9 w-full min-w-[200px] max-w-[420px] basis-1"
                  key={index}
                >
                  <Image
                    src={"/" + project.image}
                    ...
                    width={148}
                    height={148}
                    className="rounded-tl-[48px] rounded-tr-[24px] rounded-br-[48px] rounded-bl-[24px] drop-shadow-hard-purple"
                  />
                  <div className="flex flex-col gap-3">
                    <h3 className="text-b12-purple text-4xl font-headings">
                      {project.name}
                    </h3>
                    ...
                    <div className="flex flex-row gap-4">
                      {project.website ? (
                        <a
                          ...
                          className="flex flex-row-reverse items-center justify-center gap-1 text-b12-teal font-medium"
                        >
        ...
        <div className="w-full relative pb-12" id="get-involved">
          ...
          <div className="flex flex-col items-center gap-10 p-6">
            <div className="flex flex-col items-center gap-10 bg-white relative max-w-lg p-8 rounded-tl-[24px] rounded-tr-[48px] rounded-br-[24px] rounded-bl-[48px] lg:rounded-tl-[48px] lg:rounded-tr-[96px] lg:rounded-br-[48px] lg:rounded-bl-[96px] lg:p-16 drop-shadow-2xl">
              ...
              <span>
                Interested in pushing the standard forward or learning how to
                adopt? Here’s where the action happens:
              </span>
